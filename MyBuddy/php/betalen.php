<?php
  ...
  try {
    $sql = 'SELECT users.user_id, users.balans, leden.id, groepsbetalingen.bedrag, groepsbetalingen.id FROM users INNER JOIN leden ON leden.user = :user_id INNER JOIN groepsbetalingen ON groepsbetalingen.van_lid = leden.id WHERE users.user_id = :user_id AND groepsbetalingen.id = :id';
